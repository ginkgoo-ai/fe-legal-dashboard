...
function PurePilotRunningHeader(props: PilotRunningHeaderProps) {
  ...
  return (
    <div className="relative w-full flex justify-center items-center overflow-hidden rounded-lg flex-[0_0_auto]">
      <div
        className={cn(
          'absolute top-[50%] left-[50%] -translate-1/2 w-[200%] pb-[100%] rounded-lg overflow-hidden',
          {
            'animate-spin':
              pilotInfo?.pilotStatus !== PilotStatusEnum.COMPLETED && isRunning,
            'bg-linear-[125deg,#F2E0ED_5%,#E2EDEC_10%,#C9FAED_30%,#8C83E6_45%,#F4CEE4_75%,#DCC1E0_90%,teal]':
              pilotInfo?.pilotStatus !== PilotStatusEnum.COMPLETED,
            'bg-[#CAF4D0]': pilotInfo?.pilotStatus === PilotStatusEnum.COMPLETED,
          }
        )}
      ></div>
        ...
        <div className="flex flex-col bg-white rounded-lg overflow-hidden box-border p-3 gap-1.5 w-full">
          <div className="flex flex-row justify-between items-start gap-3">
            ...
            <div className="flex flex-1 flex-col justify-start items-start w-0">
              <div className="text-[#1F2937] text-lg truncate w-full">
                {pilotInfo?.pilotStatus === PilotStatusEnum.COMPLETED
